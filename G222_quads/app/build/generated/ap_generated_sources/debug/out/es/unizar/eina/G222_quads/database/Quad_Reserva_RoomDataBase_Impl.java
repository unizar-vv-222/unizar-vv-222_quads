...
public final class Quad_Reserva_RoomDataBase_Impl extends Quad_Reserva_RoomDataBase {
  ...
  protected RoomOpenDelegate createOpenDelegate() {
    final RoomOpenDelegate _openDelegate = new RoomOpenDelegate(7, "043dd7cd8a6ef96a4060607f3deafed7", "8e3f3f421cf2673766a629cc2e5797a8") {
      @Override
      public void createAllTables(@NonNull final SQLiteConnection connection) {
        SQLite.execSQL(connection, "CREATE TABLE IF NOT EXISTS `quad` (`matricula` TEXT NOT NULL, `tipo` INTEGER NOT NULL, `precio` REAL NOT NULL, `descripcion` TEXT NOT NULL, PRIMARY KEY(`matricula`))");
        SQLite.execSQL(connection, "CREATE TABLE IF NOT EXISTS `reserva` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `nombreCliente` TEXT NOT NULL, `movilCliente` TEXT NOT NULL, `fechaRecogida` INTEGER NOT NULL, `horaRecogida` INTEGER NOT NULL, `recogidaComparable` INTEGER NOT NULL, `fechaDevolucion` INTEGER NOT NULL, `horaDevolucion` INTEGER NOT NULL, `devolucionComparable` INTEGER NOT NULL, `precioTotal` REAL NOT NULL)");
        SQLite.execSQL(connection, "CREATE TABLE IF NOT EXISTS `reserva_quad_cascos` (`reservaId` INTEGER NOT NULL, `matriculaQuad` TEXT NOT NULL, `numCascos` INTEGER NOT NULL, `precioOriginal` REAL NOT NULL, PRIMARY KEY(`reservaId`, `matriculaQuad`), FOREIGN KEY(`reservaId`) REFERENCES `reserva`(`id`) ON UPDATE NO ACTION ON DELETE CASCADE , FOREIGN KEY(`matriculaQuad`) REFERENCES `quad`(`matricula`) ON UPDATE NO ACTION ON DELETE CASCADE )");
        SQLite.execSQL(connection, "CREATE INDEX IF NOT EXISTS `index_reserva_quad_cascos_reservaId` ON `reserva_quad_cascos` (`reservaId`)");
        SQLite.execSQL(connection, "CREATE INDEX IF NOT EXISTS `index_reserva_quad_cascos_matriculaQuad` ON `reserva_quad_cascos` (`matriculaQuad`)");
        SQLite.execSQL(connection, "CREATE TABLE IF NOT EXISTS room_master_table (id INTEGER PRIMARY KEY,identity_hash TEXT)");
        SQLite.execSQL(connection, "INSERT OR REPLACE INTO room_master_table (id,identity_hash) VALUES(42, '043dd7cd8a6ef96a4060607f3deafed7')");
      }

      @Override
      public void dropAllTables(@NonNull final SQLiteConnection connection) {
        SQLite.execSQL(connection, "DROP TABLE IF EXISTS `quad`");
        SQLite.execSQL(connection, "DROP TABLE IF EXISTS `reserva`");
        SQLite.execSQL(connection, "DROP TABLE IF EXISTS `reserva_quad_cascos`");
      }
      ...
      public RoomOpenDelegate.ValidationResult onValidateSchema(
        ...
        final Map<String, TableInfo.Column> _columnsQuad = new HashMap<String, TableInfo.Column>(4);
        ...
        final Set<TableInfo.ForeignKey> _foreignKeysQuad = new HashSet<TableInfo.ForeignKey>(0);
        final Set<TableInfo.Index> _indicesQuad = new HashSet<TableInfo.Index>(0);
        final TableInfo _infoQuad = new TableInfo("quad", _columnsQuad, _foreignKeysQuad, _indicesQuad);
        final TableInfo _existingQuad = TableInfo.read(connection, "quad");
        if (!_infoQuad.equals(_existingQuad)) {
          return new RoomOpenDelegate.ValidationResult(false, "quad(es.unizar.eina.G222_quads.database.Quad).\n"
                  + " Expected:\n" + _infoQuad + "\n"
                  + " Found:\n" + _existingQuad);
        }
        final Map<String, TableInfo.Column> _columnsReserva = new HashMap<String, TableInfo.Column>(10);
        ...
        final Set<TableInfo.ForeignKey> _foreignKeysReserva = new HashSet<TableInfo.ForeignKey>(0);
        final Set<TableInfo.Index> _indicesReserva = new HashSet<TableInfo.Index>(0);
        final TableInfo _infoReserva = new TableInfo("reserva", _columnsReserva, _foreignKeysReserva, _indicesReserva);
        final TableInfo _existingReserva = TableInfo.read(connection, "reserva");
        if (!_infoReserva.equals(_existingReserva)) {
          return new RoomOpenDelegate.ValidationResult(false, "reserva(es.unizar.eina.G222_quads.database.Reserva).\n"
                  + " Expected:\n" + _infoReserva + "\n"
                  + " Found:\n" + _existingReserva);
        }
        final Map<String, TableInfo.Column> _columnsReservaQuadCascos = new HashMap<String, TableInfo.Column>(4);
        ...
        final Set<TableInfo.ForeignKey> _foreignKeysReservaQuadCascos = new HashSet<TableInfo.ForeignKey>(2);
        ...
        final Set<TableInfo.Index> _indicesReservaQuadCascos = new HashSet<TableInfo.Index>(2);
        ...
        final TableInfo _existingReservaQuadCascos = TableInfo.read(connection, "reserva_quad_cascos");
        if (!_infoReservaQuadCascos.equals(_existingReservaQuadCascos)) {
          return new RoomOpenDelegate.ValidationResult(false, "reserva_quad_cascos(es.unizar.eina.G222_quads.database.ReservaQuadCascos).\n"
  ...
  public void clearAllTables() {
    super.performClear(true, "quad", "reserva", "reserva_quad_cascos");
  }
